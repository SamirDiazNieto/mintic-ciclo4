			...
			.then(
				(result) => {
					...
					result.data.getUserById.projects.map((value) => {
						...
						if (value.state === true) value.state = 'Activo';
						else {
				...
				swalWithBootstrapButtons
					...
					.then((result) => {
						if (result.isConfirmed) {
							...
							swalWithBootstrapButtons.fire('Borrada!', 'El proyecto ha sido borrado', 'success');
						} else if (
							/* Read more about handling dismissals below */
							result.dismiss === Swal.DismissReason.cancel
						) {
							swalWithBootstrapButtons.fire('Cancelado', 'El proyecto no fue borrado :)', 'error');
						}
					});
			}
	...
	const FinalizarProyecto = (datoID) => {
		arregloProyectos.map((registro) => {
			if (datoID.target.id === registro._id) {
				...
				swalWithBootstrapButtons.fire('OK!', 'El proyecto ha sido Terminado', 'success');
			}
			return;
		});
	};
		...
		arregloProyectos.map((registro) => {
			if (datoID.target.id === registro._id) {
				...
				if (registro.state == 'Activo') {
					...
					swalWithBootstrapButtons.fire('OK!', 'El proyecto ha sido inactivado', 'success');
				} else {
					ActivarProyecto(registro._id, true);
					swalWithBootstrapButtons.fire('OK!', 'El proyecto ha sido aprobado', 'success');
				}
			}
			return;
		});
	...
	const inscribir = (datoID) => {
		arregloProyectos.map((registro) => {
			if (datoID.target.id === registro._id) {
				...
				swalWithBootstrapButtons.fire('Inscrito!', 'El estudiante ha sido inscrito', 'success');
			}
			return;
		});
	};
				...
				swalWithBootstrapButtons
					...
					.then((result) => {
						if (result.value) {
							...
							swalWithBootstrapButtons.fire('Creado!', 'Avance creado correctamente', 'success');
						}
					});
			}
			return;
	...
	function CarsFilter({ preGlobalFilteredRows, globalFilter, setGlobalFilter }) {
		...
		return (
			<span className='cars-filter'>
				Buscar Proyecto:
				<input size={50} value={value || ''} onChange={handleInputChange} placeholder={`${totalCarsAvailable} Proyectos disponibles...`} />
			</span>
		);
	}
	//console.log("fin")
